...
✅ ДОБАВЛЕНО: Обработчики платной подписки
"""
...
from .handlers import (
    ...
    register_paid_subscription_handlers  # ✅ НОВОЕ: Обработчики платной подписки
)
...
"""
    ...
    async def start(self):
        """Запуск бота"""
        try:
            logger.info("Starting user bot with UNIFIED AI APPROACH + PAID SUBSCRIPTION", 
                       bot_id=self.bot_id, 
                       bot_username=self.bot_username,
                       owner_user_id=self.owner_user_id)
            ...
            # ✅ КРИТИЧНО: Регистрируем обработчики с объединенным подходом ИИ + платная подписка
            ...
            logger.info(
                "✅ User bot started with UNIFIED AI APPROACH + PAID SUBSCRIPTION", 
                bot_id=self.bot_id,
                bot_username=self.bot_username,
                owner_user_id=self.owner_user_id,
                ...
                paid_subscription_enabled=True,  # ✅ НОВОЕ
    ...
    def _setup_handlers(self):
        """✅ ОБНОВЛЕНО: Setup handlers with OWNER ACCESS CONTROL + UNIFIED AI APPROACH + MESSAGE LIMITS + PAID SUBSCRIPTION"""
        logger.info("🎯 Setting up handlers with OWNER ACCESS CONTROL + UNIFIED AI APPROACH + MESSAGE LIMITS + PAID SUBSCRIPTION", bot_id=self.bot_id)
        ...
        try:
            ...
            # ===== ✅ ОБЪЕДИНЁННЫЙ ПОРЯДОК РЕГИСТРАЦИИ С КОНТРОЛЕМ ДОСТУПА + ПЛАТНАЯ ПОДПИСКА =====
            ...
            # 💎 2. ПЛАТНАЯ ПОДПИСКА - ВЫСОКИЙ ПРИОРИТЕТ (С КОНТРОЛЕМ ДОСТУПА)
            logger.info("2️⃣ Registering PAID SUBSCRIPTION handlers with OWNER ACCESS CONTROL", bot_id=self.bot_id)
            register_paid_subscription_handlers(self.dp, **deps)
            
            # 🤖 3. ВСЯ ИИ ЛОГИКА - АДМИН + ПОЛЬЗОВАТЕЛИ (ОБЪЕДИНЕНО!) (С КОНТРОЛЕМ ДОСТУПА)
            logger.info("3️⃣ Registering UNIFIED AI handlers with OWNER ACCESS CONTROL (ADMIN + USERS IN ONE)", bot_id=self.bot_id)
            register_ai_handlers(self.dp, **deps)
            
            # 📊 4. ЛИМИТЫ СООБЩЕНИЙ (С КОНТРОЛЕМ ДОСТУПА)
            logger.info("4️⃣ Registering MESSAGE LIMIT handlers with OWNER ACCESS CONTROL", bot_id=self.bot_id)
            register_message_limit_handlers(self.dp, **deps)
            
            # 📝 5. КОНТЕНТ ОБРАБОТЧИКИ (С КОНТРОЛЕМ ДОСТУПА)
            logger.info("5️⃣ Registering content handlers with OWNER ACCESS CONTROL", bot_id=self.bot_id)
            register_content_handlers(self.dp, **deps)
            
            # ⚙️ 6. НАСТРОЙКИ ОБРАБОТЧИКИ (С КОНТРОЛЕМ ДОСТУПА)
            logger.info("6️⃣ Registering settings handlers with OWNER ACCESS CONTROL", bot_id=self.bot_id)
            register_settings_handlers(self.dp, **deps)
            
            # 📊 7. ВОРОНКА ОБРАБОТЧИКИ (С КОНТРОЛЕМ ДОСТУПА)
            logger.info("7️⃣ Registering funnel handlers with OWNER ACCESS CONTROL", bot_id=self.bot_id)
            register_funnel_handlers(self.dp, **deps)
            
            # 📨 8. МАССОВЫЕ РАССЫЛКИ (С КОНТРОЛЕМ ДОСТУПА)
            logger.info("8️⃣ Registering mass broadcast handlers with OWNER ACCESS CONTROL", bot_id=self.bot_id)
            register_mass_broadcast_handlers(self.dp, **deps)
            
            # 🎪 9. СОБЫТИЯ КАНАЛА - ТОЛЬКО СОБЫТИЯ (БЕЗ ИИ ЛОГИКИ!) (С КОНТРОЛЕМ ДОСТУПА)
            logger.info("9️⃣ Registering channel handlers with OWNER ACCESS CONTROL (EVENTS ONLY - NO AI LOGIC)", bot_id=self.bot_id)
            register_channel_handlers(self.dp, **deps)
            
            logger.info("✅ ALL HANDLERS REGISTERED WITH OWNER ACCESS CONTROL + UNIFIED AI APPROACH + MESSAGE LIMITS + PAID SUBSCRIPTION!", 
                       bot_id=self.bot_id,
                       owner_user_id=self.owner_user_id,
                       total_handlers=9,  # ✅ ОБНОВЛЕНО: Теперь 9 блоков обработчиков
                       access_control_enabled=True,  # ✅ НОВОЕ
                       admin_first=True,
                       paid_subscription_second=True,  # ✅ НОВОЕ
                       ai_unified_third=True,
                       message_limits_fourth=True,
                       channel_events_last=True,
                       architecture="UNIFIED_AI_WITH_ACCESS_CONTROL_AND_PAID_SUBSCRIPTION")  # ✅ ОБНОВЛЕНО
            
        except Exception as e:
            logger.error("💥 CRITICAL: Failed to setup unified handlers with owner access control and paid subscription", 
                        bot_id=self.bot_id, 
                        owner_user_id=self.owner_user_id,
                        error=str(e), 
                        exc_info=True)

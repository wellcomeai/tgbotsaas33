...
class UserBot:
    ...
    def _setup_handlers(self):
        """✅ ОБЪЕДИНЁННЫЙ ПОДХОД: Вся ИИ логика в ai_handlers.py"""
        logger.info("🎯 Setting up handlers with UNIFIED AI APPROACH", bot_id=self.bot_id)
        ...
        try:
            ...
            # ===== ✅ ОБЪЕДИНЁННЫЙ ПОРЯДОК РЕГИСТРАЦИИ =====
            
            # 🏆 1. АДМИН ОБРАБОТЧИКИ - ВЫСШИЙ ПРИОРИТЕТ
            logger.info("1️⃣ Registering ADMIN handlers (HIGHEST PRIORITY)", bot_id=self.bot_id)
            register_admin_handlers(self.dp, **deps)
            
            # 🤖 2. ВСЯ ИИ ЛОГИКА - АДМИН + ПОЛЬЗОВАТЕЛИ (ОБЪЕДИНЕНО!)
            logger.info("2️⃣ Registering UNIFIED AI handlers (ADMIN + USERS IN ONE)", bot_id=self.bot_id)
            register_ai_handlers(self.dp, **deps)
            
            # 📝 3. КОНТЕНТ ОБРАБОТЧИКИ
            logger.info("3️⃣ Registering content handlers", bot_id=self.bot_id)
            register_content_handlers(self.dp, **deps)
            
            # ⚙️ 4. НАСТРОЙКИ ОБРАБОТЧИКИ  
            logger.info("4️⃣ Registering settings handlers", bot_id=self.bot_id)
            register_settings_handlers(self.dp, **deps)
            
            # 📊 5. ВОРОНКА ОБРАБОТЧИКИ
            logger.info("5️⃣ Registering funnel handlers", bot_id=self.bot_id)
            register_funnel_handlers(self.dp, **deps)
            
            # 📨 6. МАССОВЫЕ РАССЫЛКИ
            logger.info("6️⃣ Registering mass broadcast handlers", bot_id=self.bot_id)
            register_mass_broadcast_handlers(self.dp, **deps)
            
            # 🎪 7. СОБЫТИЯ КАНАЛА - ТОЛЬКО СОБЫТИЯ (БЕЗ ИИ ЛОГИКИ!)
            logger.info("7️⃣ Registering channel handlers (EVENTS ONLY - NO AI LOGIC)", bot_id=self.bot_id)
            register_channel_handlers(self.dp, **deps)
            
            logger.info("✅ ALL HANDLERS REGISTERED WITH UNIFIED AI APPROACH!", 
                       bot_id=self.bot_id,
                       total_handlers=7,
                       admin_first=True,
                       ai_unified_second=True,
                       channel_events_last=True,
                       architecture="UNIFIED_AI")
            
        except Exception as e:
            logger.error("💥 CRITICAL: Failed to setup unified handlers", 
                        bot_id=self.bot_id, error=str(e), exc_info=True)
            raise

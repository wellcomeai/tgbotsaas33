...
class UserBot:
    ...
    def __init__(self, config: dict, bot_manager: 'BotManager'):
        ...
        self.ai_assistant_enabled = config.get('ai_assistant_enabled', False)
    ...
    async def start(self):
        """Запуск бота"""
        try:
            logger.info("Starting user bot with modular structure", 
                       bot_id=self.bot_id, 
                       bot_username=self.bot_username,
                       owner_user_id=self.owner_user_id)
            ...
            logger.info(
                "User bot started with modular handlers", 
                bot_id=self.bot_id,
                bot_username=self.bot_username,
                owner_user_id=self.owner_user_id,
    ...
    def _setup_handlers(self):
        ...
        logger.info("Setting up modular handlers", bot_id=self.bot_id)
        ...
        try:
            ...
            deps = {
                ...
                'ai_assistant': ai_client if self.ai_assistant_enabled else None,
                'formatter': MessageFormatter(),  # Добавляем formatter
                'user_bot': self  # Передаем ссылку на сам UserBot
            }
            
            # Регистрируем обработчики в правильном порядке с отладкой
            ...
            logger.info("All modular handlers registered successfully", bot_id=self.bot_id)
    ...
    async def update_ai_settings(self, **kwargs):
        ...
        if 'ai_assistant_enabled' in kwargs:
            self.ai_assistant_enabled = kwargs['ai_assistant_enabled']
    ...
    def update_config(self, new_config: dict):
        ...
        self.ai_assistant_enabled = new_config.get('ai_assistant_enabled', False)

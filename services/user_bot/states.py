...
✅ ОБНОВЛЕНО: Добавлены состояния для платных подписок
"""
...
class PaidSubscriptionStates(StatesGroup):
    """Состояния настройки платных подписок"""
    
    # ===== НАСТРОЙКА ROBOKASSA =====
    waiting_for_merchant_login = State()      # Ожидание логина магазина Robokassa
    waiting_for_password1 = State()           # Ожидание пароля #1 Robokassa
    waiting_for_password2 = State()           # Ожидание пароля #2 Robokassa
    
    # ===== НАСТРОЙКА ПОДПИСКИ =====
    waiting_for_subscription_price = State()  # Ожидание цены подписки
    waiting_for_subscription_period = State() # Ожидание периода подписки
    
    # ===== НАСТРОЙКА КАНАЛА =====
    waiting_for_target_channel = State()      # Ожидание целевого канала для подписки
    
    # ===== НАСТРОЙКА СООБЩЕНИЙ =====
    waiting_for_success_message = State()     # Ожидание сообщения об успешной оплате
    waiting_for_expired_message = State()     # Ожидание сообщения об окончании подписки
    waiting_for_reminder_message = State()    # Ожидание напоминающего сообщения
...
__all__ = [
    ...
    'BroadcastStates',
    'ChannelStates',
    'PaidSubscriptionStates',  # ✅ НОВОЕ: Состояния платных подписок
    'TokenStates',
    'AnalyticsStates',
    'DebugStates',
    'TemporaryStates',
...
НОВЫЕ ВОЗМОЖНОСТИ ПЛАТНЫХ ПОДПИСОК:
✅ Интеграция с Robokassa
✅ Настройка периодов и цен подписки

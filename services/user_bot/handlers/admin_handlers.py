...
✅ ДОБАВЛЕНО: Интеграция с системой платных подписок
"""
...
def register_admin_handlers(dp: Dispatcher, **kwargs):
    ...
    try:
        ...
        # ===== ✅ НОВОЕ: ПЛАТНЫЕ ПОДПИСКИ =====
        dp.callback_query.register(handler.cb_admin_paid_subscription, F.data == "admin_paid_subscription")
        ...
        logger.info("✅ Admin handlers registered successfully with AI integration, message limits, access control, paid subscriptions and TokenManager integration", 
                   bot_id=bot_config['bot_id'], 
                   owner_id=bot_config['owner_user_id'],
                   ai_handlers_count=9,
                   message_limit_handlers=4,
                   paid_subscription_handlers=1,
                   total_handlers=24,
...
class AdminHandler:
    """Класс обработчиков администратора с полной интеграцией ИИ, лимитов сообщений и платных подписок"""
    ...
    def __init__(self, db, bot_config: dict, funnel_manager, user_bot, access_control=None):
        ...
        # ✅ НОВОЕ: Инициализация PaidSubscriptionManager
        self.paid_subscription_manager = None
    ...
    async def _get_paid_subscription_manager(self):
        """Ленивая инициализация менеджера платных подписок"""
        if self.paid_subscription_manager is None:
            try:
                from .paid_subscription_handlers import PaidSubscriptionManager
                self.paid_subscription_manager = PaidSubscriptionManager(
                    db=self.db,
                    bot_config=self.bot_config
                )
                logger.debug("✅ PaidSubscriptionManager initialized", bot_id=self.bot_id)
            except Exception as e:
                logger.error("💥 Failed to initialize PaidSubscriptionManager", error=str(e))
                return None
        return self.paid_subscription_manager
...
"""
    ...
    async def cb_admin_paid_subscription(self, callback: CallbackQuery):
        """Главное меню платных подписок"""
        if self.access_control:
            has_access, status = await self.access_control['check_owner_access']()
            if not has_access:
                await self.access_control['send_access_denied'](callback, status)
        ...
        try:
            manager = await self._get_paid_subscription_manager()
            if manager:
                await manager.show_main_menu(callback)
            else:
                await callback.answer("❌ Ошибка инициализации менеджера платных подписок", show_alert=True)
        except Exception as e:
            logger.error("💥 Error in paid subscription menu", error=str(e))
            await callback.answer("❌ Ошибка при загрузке", show_alert=True)
...
"""
    ...
    async def debug_owner_message(self, message: Message):
        ...
        await message.answer(
            ...
            f"Paid Subscription Manager Initialized: {self.paid_subscription_manager is not None}\n"

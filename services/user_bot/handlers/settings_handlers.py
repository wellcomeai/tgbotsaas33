...
def register_settings_handlers(dp: Dispatcher, **kwargs):
    ...
    try:
        ...
        logger.info("Settings handlers registered successfully with entities support", 
...
"""
    ...
    async def handle_welcome_message_input(self, message: Message, state: FSMContext):
        """✅ ОБНОВЛЕНО: Обработка ввода приветственного сообщения с поддержкой форматирования"""
        ...
        try:
            # ✅ КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: Используем html_text вместо text
            formatted_text = message.html_text if message.html_text else message.text
            
            # Обновляем в БД
            await self.db.update_welcome_settings(self.bot_id, welcome_message=formatted_text)
            
            # ✅ ИСПРАВЛЕНО: Обновляем в текущей конфигурации
            self.welcome_message = formatted_text
            
            # ✅ ИСПРАВЛЕНО: Обновляем в UserBot
            if self.user_bot:
                await self.user_bot.update_welcome_settings(welcome_message=formatted_text)
            
            # ✅ ИСПРАВЛЕНО: Обновляем bot_manager
            if self.bot_config.get('bot_manager'):
                await self.bot_config['bot_manager'].update_bot_config(
                    self.bot_id,
                    welcome_message=formatted_text
                )
            
            example = self.formatter.format_message_template(formatted_text, username="@example_user", first_name="Иван")
            
            success_text = f"""
✅ <b>Приветственное сообщение обновлено!</b>

<b>Ваше сообщение (с форматированием):</b>
{formatted_text}

<b>Пример отображения для пользователя:</b>
{example}

💡 <i>Форматирование сохранено и будет отображаться у пользователей!</i>
"""
            ...
            logger.info("Welcome message updated with formatting", 
                       bot_id=self.bot_id, 
...
"""
    ...
    async def handle_confirmation_message_input(self, message: Message, state: FSMContext):
        """✅ ОБНОВЛЕНО: Обработка ввода сообщения подтверждения с поддержкой форматирования"""
        if not self._is_owner(message.from_user.id):
            return
        ...
        try:
            ...
            # Обновляем в БД
            await self.db.update_welcome_settings(self.bot_id, confirmation_message=formatted_text)
            
            # ✅ ИСПРАВЛЕНО: Обновляем в текущей конфигурации
            self.confirmation_message = formatted_text
            
            # ✅ ИСПРАВЛЕНО: Обновляем в UserBot
            if self.user_bot:
                await self.user_bot.update_welcome_settings(confirmation_message=formatted_text)
            
            # ✅ ИСПРАВЛЕНО: Обновляем bot_manager
            if self.bot_config.get('bot_manager'):
                await self.bot_config['bot_manager'].update_bot_config(
                    self.bot_id,
                    confirmation_message=formatted_text
                )
...
"""
            ...
            logger.info("Confirmation message updated with formatting", 
                       bot_id=self.bot_id,
                       has_html=bool(message.html_text))
    ...
    async def handle_goodbye_message_input(self, message: Message, state: FSMContext):
        """✅ ОБНОВЛЕНО: Обработка ввода прощального сообщения с поддержкой форматирования"""
        if not self._is_owner(message.from_user.id):
            return
        ...
        try:
            ...
            # Обновляем в БД
            await self.db.update_goodbye_settings(self.bot_id, goodbye_message=formatted_text)
            
            # ✅ ИСПРАВЛЕНО: Обновляем в текущей конфигурации
            self.goodbye_message = formatted_text
            
            # ✅ ИСПРАВЛЕНО: Обновляем в UserBot
            if self.user_bot:
                await self.user_bot.update_goodbye_settings(goodbye_message=formatted_text)
            
            # ✅ ИСПРАВЛЕНО: Обновляем bot_manager
            if self.bot_config.get('bot_manager'):
                await self.bot_config['bot_manager'].update_bot_config(
                    self.bot_id,
                    goodbye_message=formatted_text
                )
...
"""
            ...
            logger.info("Goodbye message updated with formatting", 
                       bot_id=self.bot_id,
                       has_html=bool(message.html_text))

...
"""
    ...
    async def handle_upload_files(self, callback: CallbackQuery, is_owner_check):
        """Управление загрузкой файлов для file_search"""
        await callback.answer()
        ...
        try:
            ...
            files_info = ""
            if bot_vector_store:
                total_files = bot_vector_store['file_counts'].get('total', 0)
                processed_files = bot_vector_store['file_counts'].get('completed', 0)
...
"""
            ...
            # Кнопка управления файлами если есть vector store
            if bot_vector_store:
                keyboard_buttons.append([
                    InlineKeyboardButton(text="📋 Управление файлами", callback_data="openai_manage_files")
                ])
            ...
            logger.error("💥 Error showing upload files menu", error=str(e))
...
"""
        ...
        try:
            ...
            if success:
                ...
                await progress_msg.edit_text(
                    ...
                    f"Отправьте следующий файл или нажмите \"✅ Завершить загрузку\"",
                    reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(text="✅ Завершить загрузку", callback_data="openai_finish_upload")],
                        [InlineKeyboardButton(text="❌ Отмена", callback_data="openai_upload_files")]
                    ])
    ...
    async def handle_finish_upload(self, callback: CallbackQuery, state: FSMContext, is_owner_check):
        """Завершение загрузки и создание vector store"""
        await callback.answer()
        ...
        try:
            ...
            if success:
                # Сохраняем vector_store_id в настройки агента
                await self._save_vector_store_id(vector_store_id)
                
                # Включаем file_search инструмент
                await self._enable_file_search_tool(vector_store_id)
...
"""
            ...
            logger.error("💥 Error finishing upload", error=str(e))
            await callback.message.edit_text("❌ Произошла ошибка при создании базы знаний")
            await state.clear()

    async def _save_vector_store_id(self, vector_store_id: str):
        """Сохранение vector_store_id в настройки агента"""
        try:
            current_settings = self.ai_assistant_settings.copy()
            openai_settings = current_settings.get('openai_settings', {})
            
            # Добавляем vector_store_id
            openai_settings['vector_store_ids'] = [vector_store_id]
            current_settings['openai_settings'] = openai_settings
            
            # Сохраняем в БД
            await self.db.update_ai_assistant(self.bot_id, settings=current_settings)
            
            # Обновляем локальные настройки
            self.ai_assistant_settings = current_settings
            
            logger.info("✅ Vector store ID saved", vector_store_id=vector_store_id)
            
        except Exception as e:
            logger.error("💥 Failed to save vector store ID", error=str(e))

    async def _enable_file_search_tool(self, vector_store_id: str):
        """Включение file_search инструмента"""
        try:
            current_settings = self.ai_assistant_settings.copy()
            openai_settings = current_settings.get('openai_settings', {})
            
            # Включаем file_search
            openai_settings['enable_file_search'] = True
            openai_settings['vector_store_ids'] = [vector_store_id]
            ...
            logger.info("✅ File search tool enabled", vector_store_id=vector_store_id)
            
        except Exception as e:
            logger.error("💥 Failed to enable file search", error=str(e))
    ...
    async def handle_manage_files(self, callback: CallbackQuery, is_owner_check):
        ...
        text = """
📋 <b>Управление файлами базы знаний</b>
...
"""
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📤 Загрузить еще файлы", callback_data="openai_start_upload")],
            [InlineKeyboardButton(text="🗑️ Очистить базу знаний", callback_data="openai_clear_knowledge")],

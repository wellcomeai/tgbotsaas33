...
"""
    ...
    async def handle_finish_upload(self, callback: CallbackQuery, state: FSMContext, is_owner_check):
        """✅ ИСПРАВЛЕНО: Завершение загрузки с правильным сохранением vector store"""
        await callback.answer()
        ...
        try:
            ...
            if success:
                # ✅ ИСПРАВЛЕНО: Используем объединенный метод сохранения
                save_success = await self._save_and_enable_vector_store(vector_store_id)
...
✅ Агент готов к работе с файлами""",
    ...
    async def handle_confirm_clear_knowledge(self, callback: CallbackQuery, is_owner_check):
        """✅ НОВЫЙ: Подтверждение очистки базы знаний"""
        await callback.answer()
...
🗑️ Удалено vector stores: {len(vector_store_ids)}
📁 Все файлы удалены из OpenAI
🔄 File Search выключен

    ...
    async def handle_confirm_clear_knowledge(self, callback: CallbackQuery, is_owner_check):
        ...
        try:
            ...
            if vector_store_ids:
                ...
                # ✅ ИЗМЕНЕНИЕ 4: Очищаем из БД И удаляем информацию о файлах
                clear_success = await self._clear_vector_stores_and_files()
...
Можете загрузить новые файлы.""",
    ...
    async def _clear_vector_stores_and_files(self) -> bool:
        """✅ ИЗМЕНЕНИЕ 5: НОВЫЙ метод полной очистки vector stores + информации о файлах"""
        try:
            logger.info("🗑️ Clearing vector stores and files info", bot_id=self.bot_id)
            
            # 1. Очищаем vector stores через AIManager (как раньше)
            from database.managers.ai_manager import AIManager
            ...
            # 2. НОВОЕ: Очищаем информацию о файлах в settings
            settings = self.ai_assistant_settings.copy()
            openai_settings = settings.get('openai_settings', {})
            ...
            logger.info("✅ Vector stores and files info cleared", 
                       cleared_files_count=files_count)
            
            return True
            
        except Exception as e:
            logger.error("💥 Failed to clear vector stores and files", 
                        error=str(e), exc_info=True)
            return False

...
class Settings(BaseSettings):
    ...
    # ✅ NEW: Referral Program Settings
    referral_commission_rate: float = Field(default=15.0, env="REFERRAL_COMMISSION_RATE", description="Commission rate percentage (15%)")
    referral_enabled: bool = Field(default=True, env="REFERRAL_ENABLED", description="Enable referral program")
    referral_min_payout: float = Field(default=100.0, env="REFERRAL_MIN_PAYOUT", description="Minimum payout amount in rubles")
    referral_link_format: str = Field(default="t.me/{bot_username}?start={referral_code}", description="Referral link format")
...
"""
    ...
    # ✅ NEW: Referral program messages
    REFERRAL_WELCOME = f"""
{Emoji.HANDSHAKE} <b>Партнерская программа AI ADMIN</b>

Приглашайте друзей и получайте <b>15% с каждой их оплаты!</b>

{Emoji.MONEY} <b>Как это работает:</b>
- Отправляете друзьям свою партнерскую ссылку
- Они регистрируются по вашей ссылке
- За каждую их оплату вы получаете 15%
- Комиссия начисляется автоматически

{Emoji.ROCKET} <b>Ваша партнерская ссылка:</b>
<code>{{referral_link}}</code>

{Emoji.CHART} <b>Ваша статистика:</b>
- Приглашено: <b>{{total_referrals}}</b> человек
- Заработано: <b>{{total_earnings}}₽</b>
- Всего комиссий: <b>{{total_commission}}₽</b>

{Emoji.INFO} <b>Совет:</b> Расскажите друзьям о возможностях создания ботов с ИИ агентами!
"""

    REFERRAL_NEW_REFERRAL = f"""
{Emoji.SUCCESS} <b>Новый реферал!</b>

К вам присоединился новый пользователь по партнерской ссылке!

{Emoji.FRIEND} <b>Пользователь:</b> {{user_info}}
{Emoji.LINK} <b>Источник:</b> Ваша реферальная ссылка

{Emoji.COMMISSION} <b>Когда он оплатит подписку или токены, вы получите 15% комиссии!</b>

{Emoji.TEAM} Всего рефералов: <b>{{total_referrals}}</b>
"""

    REFERRAL_COMMISSION_EARNED = f"""
{Emoji.EARNING} <b>Получена партнерская комиссия!</b>

{Emoji.PERCENTAGE} <b>Сумма:</b> {{commission_amount}}₽ (15%)
{Emoji.PAYMENT} <b>С оплаты:</b> {{payment_amount}}₽
{Emoji.FRIEND} <b>От реферала:</b> {{referred_user}}

{Emoji.CHART} <b>Ваша статистика:</b>
- Общий доход: <b>{{total_earnings}}₽</b>
- Всего рефералов: <b>{{total_referrals}}</b>

{Emoji.ROCKET} Продолжайте приглашать друзей и зарабатывать!
"""

    REFERRAL_PAYOUT_REQUEST = f"""
{Emoji.BANK} <b>Запрос на выплату</b>

Минимальная сумма для вывода: <b>{settings.referral_min_payout}₽</b>

{Emoji.MONEY} <b>Ваш баланс:</b> {{current_balance}}₽
{Emoji.RECEIPT} <b>Доступно к выводу:</b> {{available_amount}}₽

{Emoji.INFO} <b>Для получения выплаты:</b>
1. Убедитесь что баланс больше {settings.referral_min_payout}₽
2. Обратитесь в поддержку с запросом
3. Укажите реквизиты для получения

{Emoji.WARNING} <b>Выплаты обрабатываются в течение 3-5 рабочих дней</b>
"""

    REFERRAL_INSUFFICIENT_BALANCE = f"""
{Emoji.WARNING} <b>Недостаточно средств для вывода</b>

{Emoji.MONEY} <b>Ваш баланс:</b> {{current_balance}}₽
{Emoji.RECEIPT} <b>Минимум для вывода:</b> {settings.referral_min_payout}₽
{Emoji.PERCENTAGE} <b>Нужно еще:</b> {{needed_amount}}₽

{Emoji.ROCKET} <b>Как увеличить доходы:</b>
• Приглашайте больше активных пользователей
• Делитесь ссылкой в соцсетях и чатах
• Рассказывайте о возможностях платформы

{Emoji.INFO} Каждая оплата реферала принесет вам 15% комиссии!
"""

    REFERRAL_DISABLED = f"""
{Emoji.INFO} <b>Партнерская программа временно недоступна</b>

Приносим извинения за неудобства. Функция будет восстановлена в ближайшее время.

{Emoji.NOTIFICATION} Следите за обновлениями в новостях проекта!
"""

    REFERRAL_STATS_DETAILED = f"""
{Emoji.ANALYTICS} <b>Детальная статистика партнерской программы</b>

{Emoji.TEAM} <b>Рефералы:</b>
• Всего приглашено: <b>{{total_referrals}}</b>
• Активных (оплачивали): <b>{{active_referrals}}</b>
• Новых за месяц: <b>{{monthly_referrals}}</b>

{Emoji.PROFIT} <b>Доходы:</b>
• Общий доход: <b>{{total_earnings}}₽</b>
• За текущий месяц: <b>{{monthly_earnings}}₽</b>
• Средняя комиссия: <b>{{average_commission}}₽</b>

{Emoji.PERCENTAGE} <b>Конверсия:</b>
• Процент оплачивающих: <b>{{conversion_rate}}%</b>
• Средний чек реферала: <b>{{average_payment}}₽</b>

{Emoji.BANK} <b>Баланс:</b>
• Доступно к выводу: <b>{{available_balance}}₽</b>
• В ожидании выплаты: <b>{{pending_payout}}₽</b>

{Emoji.FIRE} <b>Ваша эффективность выше среднего!</b> Продолжайте в том же духе.
"""

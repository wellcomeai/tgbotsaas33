...
class ContentManager:
    ...
    async def save_rewrite_result(self, bot_id: str, rewrite_data: Dict[str, Any]) -> bool:
        """Сохранение результата рерайта в канал"""
        try:
            async with get_db_session() as session:
                query = text("""
                UPDATE bot_admin_channels 
            ...
            logger.error("💥 Error saving rewrite result", bot_id=bot_id, error=str(e))
            return False

    async def get_last_rewrite(self, bot_id: str) -> Optional[Dict[str, Any]]:
        """Получение последнего рерайта"""
        try:
            async with get_db_session() as session:
                query = text("""
                ...
                if row and row[0]:
                    return row[0]  # JSONB автоматически deserializуется
                return None
                
        except Exception as e:
            logger.error("💥 Error getting last rewrite", bot_id=bot_id, error=str(e))
            return None

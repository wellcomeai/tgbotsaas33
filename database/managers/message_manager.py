...
class MessageManager:
    ...
    async def check_daily_message_limit(bot_id: str, user_id: int) -> Tuple[bool, int, int]:
        ...
        msk_today = datetime.now(msk_tz).replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    async def increment_daily_message_usage(bot_id: str, user_id: int) -> bool:
        ...
        try:
            ...
            usage_log = await MessageManager.get_or_create_ai_usage(bot_id, user_id, msk_today)
            
            async with get_db_session() as session:
                # Увеличиваем счетчик
                await session.execute(
                    update(usage_log.__class__)
                    .where(usage_log.__class__.id == usage_log.id)
                    .values(
                        messages_count=usage_log.__class__.messages_count + 1,
                        updated_at=datetime.now()
                    )
                )
                await session.commit()
            
            logger.debug("📈 Daily message usage incremented", 
                        bot_id=bot_id,
                        user_id=user_id)
    ...
    async def get_or_create_ai_usage(bot_id: str, user_id: int, date: datetime = None):
        ...
        async with get_db_session() as session:
            ...
            if not usage_log:
                ...
                logger.debug("📝 New AI usage log created", 
                            bot_id=bot_id,
                            user_id=user_id,
                            date=date.date())
    ...
    async def increment_ai_usage(bot_id: str, user_id: int) -> Tuple[int, Optional[int]]:
        ...
        new_count = usage_log.messages_count + 1
        
        logger.debug("📈 AI usage incremented", 
                    bot_id=bot_id,
                    user_id=user_id,
                    new_count=new_count,
                    daily_limit=daily_limit)

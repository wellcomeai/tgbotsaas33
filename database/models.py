...
# ✅ НОВЫЕ МОДЕЛИ: Платные подписки для ботов
class BotPaidSubscription(Base):
    """Настройки платных подписок для ботов"""
    __tablename__ = "bot_paid_subscriptions"
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    bot_id = Column(String(100), ForeignKey("user_bots.bot_id", ondelete="CASCADE"), nullable=False, unique=True)
    
    # Robokassa настройки для конкретного бота
    robokassa_merchant_login = Column(String(255), nullable=True)
    robokassa_password1 = Column(String(255), nullable=True)  
    robokassa_password2 = Column(String(255), nullable=True)
    robokassa_test_mode = Column(Boolean, default=True, nullable=False)
    
    # Настройки подписки
    subscription_price = Column(Numeric(precision=10, scale=2), nullable=False)
    subscription_period_days = Column(Integer, default=30, nullable=False)
    
    # Канал для добавления
    target_channel_id = Column(BigInteger, nullable=False)
    target_channel_username = Column(String(255), nullable=True)
    target_channel_title = Column(String(255), nullable=True)
    
    # Сообщения
    payment_success_message = Column(Text, nullable=True)
    payment_failed_message = Column(Text, nullable=True)
    subscription_expired_message = Column(Text, nullable=True)
    subscription_reminder_message = Column(Text, nullable=True)
    
    # Настройки уведомлений
    reminder_days_before = Column(Integer, default=3, nullable=False)  # За 3 дня до истечения
    ...
    # Relationships
    bot = relationship("UserBot", backref="paid_subscription_settings")

    def __repr__(self):
        return f"<BotPaidSubscription(id={self.id}, bot_id={self.bot_id}, price={self.subscription_price}, enabled={self.is_enabled})>"


class PaidSubscriber(Base):
    """Платные подписчики ботов"""
    ...
    robokassa_invoice_id = Column(String(100), nullable=True)
    
    # Канал
    channel_id = Column(BigInteger, nullable=False)
    added_to_channel_at = Column(DateTime, nullable=True)
    
    # Уведомления
    reminder_sent_at = Column(DateTime, nullable=True)
    expired_notification_sent_at = Column(DateTime, nullable=True)
    ...
    # Unique constraint
    __table_args__ = (
        UniqueConstraint('bot_id', 'user_id', name='uq_bot_paid_subscriber'),
    )
    
    # Relationships
    bot = relationship("UserBot")

    def is_active(self) -> bool:
        """Проверить активна ли подписка"""
        return self.status == "active" and self.subscription_end > datetime.now()
    ...
    def is_expiring_soon(self, days_threshold: int = 3) -> bool:
        """Истекает ли подписка в ближайшие дни"""
        return self.is_active() and self.days_until_expiry() <= days_threshold
    
    def extend_subscription(self, days: int) -> bool:
    ...
    def reactivate_subscription(self) -> bool:
        ...
        if self.status in ["cancelled", "expired"] and self.subscription_end > datetime.now():
            self.status = "active"
            return True
        return False
    
    def expire_subscription(self) -> bool:
        """Отметить подписку как истекшую"""
        if self.status == "active" and self.subscription_end <= datetime.now():
            self.status = "expired"
            return True
        return False
    ...
    def get_subscription_info(self) -> dict:
        ...
        return {
            ...
            'added_to_channel': bool(self.added_to_channel_at),
            'reminder_sent': bool(self.reminder_sent_at),
            'expired_notification_sent': bool(self.expired_notification_sent_at)
        }
    
    def __repr__(self):
        return f"<PaidSubscriber(id={self.id}, bot_id={self.bot_id}, user_id={self.user_id}, status={self.status})>"
...
__all__ = [
    ...
    "MassBroadcast", "BroadcastDelivery", "ReferralTransaction", "BotPaidSubscription", "PaidSubscriber"
]

...
class MasterBot:
    ...
    async def cmd_start(self, message: Message, state: FSMContext):
        ...
        try:
            ...
            logger.info("User registered", user_id=message.from_user.id)
...
{Emoji.NEW} <b>Новинка:</b> Каждый бот имеет собственную админ-панель!
...
{Emoji.CHART} <b>Создан:</b> {bot.created_at.strftime('%d.%m.%Y')}
...
{Emoji.WARNING} <b>Важно:</b> Сам бот в Telegram останется, но перестанет работать.
...
"""
        ...
        # Add confirmation callback handler temporarily
    ...
    async def _confirm_delete_bot(self, callback: CallbackQuery):
        ...
        try:
            ...
            await callback.message.edit_text(
                f"{Emoji.SUCCESS} <b>Бот успешно удален!</b>\n\n"
    ...
    async def start_polling(self):
        ...
        logger.info("Master bot started (simplified version)")
